...
int main (int argc, char* argv[])
{
    ...
    for ( Technique* tech: resolver.techniques ) {
        ...
        QApplication::connect(
            pCheck, &QCheckBox::clicked, pCheck,
            [tech, pCheck] ( ) {
            tech->setEnabled(pCheck->isChecked( ));
        },
            Qt::QueuedConnection);
        QApplication::connect(
            tech, &Technique::started, pCheck,
            [pCheck] ( ) {
        ...
        QApplication::connect(
            ...
            Qt::QueuedConnection);
        QApplication::connect(
            &resolver, &Resolver::newIteration, pCheck,
            [pCheck] ( ) {
            QPalette pal = pCheck->palette( );
    ...
    QApplication::connect(
        &resolver, &Resolver::done, &diag,
        [&windowTitle, &diag] (quint64 e) {
        diag.setWindowTitle(QString("%1 resolved in %2 ms").arg(windowTitle).arg(e));
    },
        Qt::QueuedConnection);
    QApplication::connect(
        &resolver, &Resolver::started, &app,
